...
def send():
    ...
    if msg != '':
        ...
        ChatBox.insert(END, "Etudiant: " + msg + '\n\n')
        ChatBox.config(foreground="#446665", font=("ubuntu", 16 ,'bold'))
    
        ints = predict_class(msg)
        res = getResponse(ints, intents)
        
        ChatBox.insert(END, "Robot: " + res + '\n\n')
...
SendButton = Button(root, font=("Verdana",14,'bold'), text="Envoyer", width="8", height=4,
                  bg="#15a6f0", activebackground="#347d9f",fg='#fff',relief='flat',command=send)
